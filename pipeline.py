...
def extract_features(df):
    ...
    # Get patient-specific baselines from data
    resting_hr_baseline = df["patient_resting_hr_baseline"].iloc[0] if "patient_resting_hr_baseline" in df.columns else 52
    
    # 4.1 Resting HR deviation from personal baseline
    df["resting_hr_delta"] = (df["resting_HR"] - resting_hr_baseline).round(2)
...
def compute_recovery(df):
    ...
    # Get patient-specific sleep baseline
    sleep_baseline = df["patient_sleep_baseline_h"].iloc[0] if "patient_sleep_baseline_h" in df.columns else 7.5
    ...
    def classify_recovery(row):
        ...
        # Sleep - compare to patient's personal baseline
        if row["sleep_duration_h"] >= sleep_baseline: score += 2
        elif row["sleep_duration_h"] >= (sleep_baseline - 1): score += 1
        else:                              score -= 1
        # Temp deviation
...
def run_full_pipeline():
    ...
    # Load synthetic daily data (generated by data_generator.py for current patient)
    output_dir = os.path.dirname(__file__)
    daily_data_path = os.path.join(output_dir, "synthetic_daily_data.csv")
    
    if os.path.exists(daily_data_path):
        df = pd.read_csv(daily_data_path)
        print(f"\n[Task 3] Loaded {len(df)} patient-specific daily records from CSV")
    else:
        # Fallback: generate with default patient if file doesn't exist
        df = generate_daily_records()
        print(f"\n[Task 3] Generated {len(df)} synthetic daily records (default patient)")
    ...
    out_df.to_csv(os.path.join(output_dir, "analytics_pipeline_output.csv"), index=False)
